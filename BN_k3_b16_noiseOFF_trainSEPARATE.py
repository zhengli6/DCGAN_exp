...
x = layers.Conv2D(channels, 3, activation='tanh', padding='same', name='g_btm_layer')(x)
generator = keras.models.Model(generator_input, x)
generator.summary()

#################### Discriminator ##############################
discriminator_input = layers.Input(shape=(height, width, channels))
x = layers.Conv2D(32, 3, strides=2, name = 'd_top_layer')(discriminator_input)
x = layers.LeakyReLU(alpha=0.2)(x)
# x = layers.Dropout(0.25)(x)

x = layers.Conv2D(64, 3, strides=2, padding='same', use_bias=True)(x)
x = layers.BatchNormalization(momentum = 0.9)(x)
x = layers.LeakyReLU(alpha=0.2)(x)
# x = layers.Dropout(0.25)(x)

x = layers.Conv2D(128, 3, strides=2, padding='same', use_bias=True)(x)
x = layers.BatchNormalization(momentum = 0.9)(x)
